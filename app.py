...
def generate_puantaj_excel(df: pd.DataFrame, year: int, month: int) -> bytes:
    ...
    cell_fmt = workbook.add_format({"border": 1, "border_color": "black"})
    weekend_cell_fmt = workbook.add_format(
        {"border": 1, "border_color": "black", "bg_color": "#FFF4CC"}
    )
    ...
    # Sütun genişlikleri: A sütunu geniş, diğerleri dar
    worksheet.set_column(0, 0, 30)  # Öğrenci sütunu
    worksheet.set_column(1, max_col - 1, 5)

    # Tüm dolu aralık için kenarlık uygula (boş hücreler dahil)
    # 0. satır başlık, 1..len(df) veri satırları
    end_row = max_row - 1
    end_col = max_col - 1
