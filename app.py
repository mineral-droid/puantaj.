...
def generate_puantaj_excel(df: pd.DataFrame, year: int, month: int) -> bytes:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        wb = writer.book
        ws = wb["Puantaj"]
        # İlk satır başlık, formüller 2. satırdan itibaren
        max_row = ws.max_row
        max_col = ws.max_column
        # Çalışma / eğitim / izin sütunları ilk 3 sütun, 5. sütundan itibaren günler
        for row in range(2, max_row + 1):
            # Gün sütun aralığı: 5. sütundan son sütuna kadar
            start_col_letter = ws.cell(row=1, column=5).column_letter
            end_col_letter = ws.cell(row=1, column=max_col).column_letter
            day_range = f"{start_col_letter}{row}:{end_col_letter}{row}"
            ws.cell(row=row, column=2).value = f"=COUNTIF({day_range},\"X\")"
            ws.cell(row=row, column=3).value = f"=COUNTIF({day_range},\"E\")"
            ws.cell(row=row, column=4).value = f"=COUNTIF({day_range},\"İ\")"
    buffer.seek(0)
    return buffer.read()
